...
class CrimeTablePage(QWidget):
    ...
    def set_line2(self):
        self.search_line2 = QLineEdit(self)
        self.search_line2.setGeometry(130, 70, 200, 20)

    def set_btn2(self):
        self.search_btn2 = QPushButton('검색', self)
        self.insert_btn2 = QPushButton('추가', self)
        self.delete_btn2 = QPushButton('삭제', self)

        self.search_btn2.setGeometry(350, 70, 40, 20)
        self.insert_btn2.setGeometry(405, 70, 40, 20)
        self.delete_btn2.setGeometry(460, 70, 40, 20)

        self.search_btn2.clicked.connect(self.total_search)
        self.delete_btn2.clicked.connect(self.total_delete)
        # 검색창 엔터시 검색 기능 실행
        self.search_line2.returnPressed.connect(self.total_search)

    # 검색한 데이터 있을 때와 없을 때에 조건을 넣은 메서드
    def total_search(self):
        # 초기화
        self.crime_table.clear()
        self.total_list.clear()

        # 평균을 나타내는 self.db_average 하나의 리스트로 데이터 합치기
        self.list_db_total=list(self.db)
        self.list_db_total.insert(6,self.db_average)
        print(self.list_db_total)

        search_word=str(self.search_line2.text())
        if bool(search_word) == False:
            self.total_list = list(self.list_db_total)
            self.total_search_view()
        else:
            for i in range(len(self.list_db_total)):
                for j in range(len(self.list_db_total[0])):
                    if search_word in str(self.list_db_total[i][j]):
                        self.total_list.append(self.list_db_total[i])
                    else :
                        pass
            self.total_search_view()

    # 검색한 데이터 보이도록 하는 메서드
    def total_search_view(self):
        ...
        self.crime_table.setHorizontalHeaderLabels(['구분', '인구(만명)', '범죄건수', '인구 1만명당 범죄건수',
                                                    ...
                                                    '1천명당 CCTV 수'])
        self.crime_table.setRowCount(len(self.total_list))
        for i in range(len(self.total_list)):
            for j in range(len(self.total_list[0])):
                self.crime_table.setItem(i, j, QTableWidgetItem(str(self.total_list[i][j])))

    def total_delete(self):
        reply = QMessageBox.question(self, '삭제', '삭제하시겠습니까?', QMessageBox.Yes | QMessageBox.No,
                                     QMessageBox.No)
        if reply == QMessageBox.Yes:
            row=self.crime_table.currentRow()
            print(row)
