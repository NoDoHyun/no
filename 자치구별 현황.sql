select a.구분, a.세대수, round(a.`인구(명)`/10000) as "인구(만명)", a.`공무원(명)`, a.`면적(제곱킬로미터)`, round(`인구(명)`/`공무원(명)`)  as "공무원 1인당 인구(명)"
, round(`인구(명)`/`면적(제곱킬로미터)`) as "인구밀도(명/제곱킬로미터)", b.폭력 + b.살인 + b.`강간-강제추행` + b.강도 + b.절도 as 범죄발생건수
, round((b.폭력 + b.살인 + b.`강간-강제추행` + b.강도 + b.절도) / a.`인구(명)` * 10000) as "인구 1만명당 범죄 발생건수"
